    ...
    def encode(self, SUPIR_VAE, image, encoder_dtype, use_tiled_vae, encoder_tile_size):
        ...
        if encoder_dtype == 'auto':
            try:
                ...
                raise AttributeError("reForge version too old, can't autodetect properly. Set your dtypes manually.")
        else:
        ...
        pbar = ldm_patched.modules.utils.ProgressBar(B)
        out = []
        for img in resized_image:

            SUPIR_VAE.to(dtype).to(device)

            autocast_condition = (dtype != torch.float32) and not ldm_patched.modules.model_management.is_device_mps(device)
            with torch.autocast(ldm_patched.modules.model_management.get_autocast_device(device), dtype=dtype) if autocast_condition else nullcontext():
...
"""
    ...
    def sample(self, SUPIR_model, latents, steps, seed, cfg_scale_end, EDM_s_churn, s_noise, positive, negative,
        ...
        out = []
        pbar = ldm_patched.modules.utils.ProgressBar(samples.shape[0])
        for i, sample in enumerate(samples):
            try:
                ...
                print("It's likely that too large of an image or batch_size for SUPIR was used,"
                      " and it has devoured all of the memory it had reserved, you may need to restart reForge. Make sure you are using tiled_vae, "
...
"""
    ...
    def condition(self, SUPIR_model, latents, positive_prompt, negative_prompt, captions=""):
        ...
        uc = []
        pbar = ldm_patched.modules.utils.ProgressBar(N)
        autocast_condition = (SUPIR_model.model.dtype != torch.float32) and not ldm_patched.modules.model_management.is_device_mps(device)
        with torch.autocast(ldm_patched.modules.model_management.get_autocast_device(device), dtype=SUPIR_model.model.dtype) if autocast_condition else nullcontext():
...
"""
    ...
    def process(self, supir_model, sdxl_model, diffusion_dtype, fp8_unet):
        ...
        if not hasattr(self, "model") or self.model is None or self.current_config != custom_config:
            ...
            pbar = ldm_patched.modules.utils.ProgressBar(5)
            ...
            try:
                ...
                sd = ldm_patched.modules.utils.state_dict_prefix_replace(sdxl_state_dict, replace_prefix, filter_keys=False)
            ...
            try:
                ...
                sd = ldm_patched.modules.utils.state_dict_prefix_replace(sd, replace_prefix2, filter_keys=True)                
    ...
    def process(self, supir_model, diffusion_dtype, fp8_unet, model, clip, vae, high_vram=False):
        ...
        if diffusion_dtype == 'auto':
            try:
                ...
                raise AttributeError("reForge version too old, can't autodecet properly. Set your dtypes manually.")
        else:
            print(f"Diffusion using {diffusion_dtype}")
            dtype = convert_dtype(diffusion_dtype)
        ...
        if not hasattr(self, "model") or self.model is None or self.current_config != custom_config:
            ...
            try:
                ...
                clip_l_sd = ldm_patched.modules.utils.state_dict_prefix_replace(clip_sd, replace_prefix, filter_keys=True)
                clip_text_config = CLIPTextConfig.from_pretrained(clip_config_path)
                self.model.conditioner.embedders[0].tokenizer = CLIPTokenizer.from_pretrained(tokenizer_path)
                with (init_empty_weights() if is_accelerate_available else nullcontext()):
                    self.model.conditioner.embedders[0].transformer = CLIPTextModel(clip_text_config)
            ...
            try:
                ...
                replace_prefix2 = {}
                replace_prefix2["conditioner.embedders.1.model."] = ""
                clip_g_sd = ldm_patched.modules.utils.state_dict_prefix_replace(clip_sd, replace_prefix2, filter_keys=True)             
    ...
    def process(self, supir_model, diffusion_dtype, fp8_unet, model, clip_l, clip_g, vae, high_vram=False):
        ...
        if not hasattr(self, "model") or self.model is None or self.current_config != custom_config:
            ...
            try:
                ...
                clip_l_sd = ldm_patched.modules.utils.state_dict_prefix_replace(clip_l_sd, replace_prefix, filter_keys=True)
                clip_text_config = CLIPTextConfig.from_pretrained(clip_config_path)
                self.model.conditioner.embedders[0].tokenizer = CLIPTokenizer.from_pretrained(tokenizer_path)
                with (init_empty_weights() if is_accelerate_available else nullcontext()):
                    self.model.conditioner.embedders[0].transformer = CLIPTextModel(clip_text_config)
            ...
            try:
                ...
                replace_prefix2 = {}
                replace_prefix2["conditioner.embedders.1.model."] = ""
                clip_g_sd = ldm_patched.modules.utils.state_dict_prefix_replace(clip_g_sd, replace_prefix2, filter_keys=True)             
                clip_g = build_text_model_from_openai_state_dict(clip_g_sd, device, cast_dtype=dtype)
                self.model.conditioner.embedders[1].model = clip_g
                self.model.conditioner.embedders[1].model.to(dtype)
                del clip_g_sd
