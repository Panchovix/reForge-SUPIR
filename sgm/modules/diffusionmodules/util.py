...
class CheckpointFunction(torch.autograd.Function):
    ...
    def backward(ctx, *output_grads):
        ...
        autocast_condition = (ctx.input_tensors.dtype == torch.float16 or ctx.input_tensors.dtype == torch.bfloat16) and not ldm_patched.modules.model_management.is_device_mps(device)
        with torch.autocast(ldm_patched.modules.model_management.get_autocast_device(device), dtype=ctx.input_tensors.dtype) if autocast_condition else nullcontext():
            # Fixes a bug where the first op in run_function modifies the
            # Tensor storage in place, which is not allowed for detach()'d
            # Tensors.
