...
class ControlWrapper(nn.Module):
    ...
    def forward(
        ...
        autocast_condition = (self.dtype == torch.float16 or self.dtype == torch.bfloat16) and not ldm_patched.modules.model_management.is_device_mps(device)
        with torch.autocast(ldm_patched.modules.model_management.get_autocast_device(device), dtype=self.dtype) if autocast_condition else nullcontext():
